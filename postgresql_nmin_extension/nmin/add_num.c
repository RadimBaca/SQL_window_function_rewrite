...
PGDLLEXPORT Datum add_numberN(PG_FUNCTION_ARGS);
PGDLLEXPORT Datum add_numberN_final(PG_FUNCTION_ARGS);

PG_FUNCTION_INFO_V1(add_number);
PG_FUNCTION_INFO_V1(add_number_final);
PG_FUNCTION_INFO_V1(add_numberN);
PG_FUNCTION_INFO_V1(add_numberN_final);
...
Datum add_number_final(PG_FUNCTION_ARGS)
{
    ...
    PG_RETURN_INT32(DatumGetInt32(input_elems[j]));
}



Datum add_numberN(PG_FUNCTION_ARGS)
{
    ArrayType* input_arr = PG_GETARG_ARRAYTYPE_P(0);
    int32 insert_var = PG_GETARG_INT32(1);
...
Datum add_numberN_final(PG_FUNCTION_ARGS)
{
    ArrayType* input_arr = PG_GETARG_ARRAYTYPE_P(0);
    int32 insert_var = PG_GETARG_INT32(1);
    int32 N = PG_GETARG_INT32(2);
    ...
    if (input_len == 1 && N > 1)
    {
        PG_RETURN_INT32(Int32GetDatum(2147483647));
    }
    deconstruct_array(input_arr, INT4OID, 4, true, 'i', &input_elems, &input_nulls, &input_len);
