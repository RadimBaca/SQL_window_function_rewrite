...
def compute_means(data, dbms, has_cost, intro_text):
    ...
    print("DBMS: " + dbms)
    print(intro_text)
    print("Mean WF strategy time: ", data['T1'].mean())
    ...
    if has_cost:
        ...
        print("Mean of best times:", better_time_data.mean())
        print("Mean of cost times:", cost_time_data.mean())
        ...
        print("Number of rows with T1 missmatch cost:", len(missmatch_cost1) / len(data) * 100, "% (",
              len(missmatch_cost1), "/", len(data), ")")
        print("Number of rows with T2 missmatch cost:", len(missmatch_cost2) / len(data) * 100, "% (",
