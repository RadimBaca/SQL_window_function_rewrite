...
def agg_analysis(dbms_list, has_cost, index):
    ...
    for (dbms,sel) in dbms_list:
        ...
        plt.plot(pb_values, division_result_list, marker='o', label=dbms + ', sel: ' + sel)
...
def without_index():
    ...
    sel = ['<1', '<32']
    result = [(d, s) for s in sel for d in dbms_list]
